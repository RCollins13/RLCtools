...
#' @param staple.col Line color used to draw bracket \[default: "black"\]
...
staple.bracket <- function(x0, x1, y0, y1, orient="vertical", staple.len=0.25,
                           staple.col="black", staple.accent.color=NULL,
                           accent.len=staple.len/2, ...){
  if(orient == "horizontal"){
    polygon(x=c(x0, x0, x1, x1), y=c(y0-accent.len, y0, y1, y1-accent.len),
            border=NA, col=staple.accent.color, xpd=T)
    segments(x0=x0, x1=x0, y0=y0, y1=y0-staple.len, xpd=T, col=staple.col, ...)
    segments(x0=x1, x1=x1, y0=y1, y1=y1-staple.len, xpd=T, col=staple.col, ...)
  }else{
    polygon(x=c(x0+accent.len, x0, x1, x1+accent.len), y=c(y0, y0, y1, y1),
            border=NA, col=staple.accent.color, xpd=T)
    segments(x0=x0, x1=x0+staple.len, y0=y0, y1=y0, xpd=T, col=staple.col, ...)
    segments(x0=x1, x1=x1+staple.len, y0=y1, y1=y1, xpd=T, col=staple.col, ...)
  }
  segments(x0=x0, x1=x1, y0=y0, y1=y1, xpd=T, col=staple.col, ...)
}
